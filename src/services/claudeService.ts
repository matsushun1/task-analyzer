...
- 1日の情報量を絞り、シンプルに保つ
- ADHDの特性として「相談すべきタイミングの判断」が難しいことを考慮する。具体的には:
  - 同じタスクに2日以上かかっている場合、「今日中に原因が分からなければ相談する」などの明示的な相談トリガーを提示する
  - 「調査しても決定打がつかめない」状況は相談すべきサインであることをアドバイスに含める
  - 相談を促す場合は「上司に相談する」ではなく「〇〇について△△を確認したい、と伝える」のように具体的な相談内容と伝え方を示す
...
- taskManagementAdvice: 抽象的な助言ではなく、明日から実行できる具体的な「次の一手」を1つ含めて100-200文字で記述（例: 「まず○○のタスクの見積もりを15分で見直す」）。同じタスクが複数日のノートに登場していたり、課題・懸念事項に同じ内容が繰り返し記録されている場合は、「今日中に解決の目処が立たなければ、〇〇について△△を確認したい、と上司に伝える」のように相談タイミングと具体的な伝え方をアドバイスに含めること。`
}
